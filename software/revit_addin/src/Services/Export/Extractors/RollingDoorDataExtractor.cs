...
{
    public class RollingDoorDataExtractor : IComponentDataExtractor
    {
           ...
           string areaOrVolumeUnit)
        {
            ...
            Add(results, "Peso aproximado",
                FindFirst(context,
                    new[] { "Weight", "Peso", "Mass", "Masa" }),
                "Parámetro Revit");

            // De momento lo mantenemos por si alguna familia lo define.
            Add(results, "Transmitancia térmica componente",
                ThermalPropertiesService.GetFamilyUValue(context.ElementType),
                "FamilyThermalProperties",
                "W/(m²·K)");

            Add(results, "Conductividad térmica materiales",
                ThermalPropertiesService.GetMaterialsConductivity(context),
                "ThermalAsset de materiales",
                "W/(m·K)");

            Add(results, "Densidad materiales",
                ThermalPropertiesService.GetMaterialsDensity(context),
                "ThermalAsset de materiales",
                "kg/m³");
        ...
        private static string FindManufacturer(RevitTypeContext context)
        {
            ...
            if (RevitExportDataBuilder.IsReasonableText(manufacturer, false))
                return manufacturer;
